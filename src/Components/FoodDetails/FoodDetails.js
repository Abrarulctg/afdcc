...
const FoodDetails = (props) => {
    ...
    useEffect(() => {
        fetch("https://hot-onion-restaurent-backend.herokuapp.com/food/" + id)
            .then(res => res.json())
            .then(data => {
                setCurrentFood(data);
                        ...
                        <div className="more-images mt-5 ">
                            {
                                currentFood.images.map((img, index) =>
                                    <img onClick={() => setSelectedBigImg(currentFood.images[index])}
                                        className={currentFood.images[index] === selectedBigImg ? "mr-4 small-img active-small-img" : "mr-4 small-img"}
                                        height="150px" src={img} alt="" />)
                            }
                        </div>
                    </div>
